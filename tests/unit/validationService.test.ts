...
describe('ValidationService', () => {
  ...
  describe('validateBuild', () => {
    ...
    it('should calculate score correctly', () => {
      ...
      expect(validation.overallScore).toBeGreaterThanOrEqual(8); // Near perfect (no item/skill data to score a full 10)
    ...
    it('should give low score for broken build', () => {
      ...
      expect(validation.overallScore).toBeLessThan(5); // low score (bleed/freeze are warnings, not criticals, in PoE2)
      expect(validation.isValid).toBe(false);
      expect(validation.criticalIssues.length).toBeGreaterThanOrEqual(4); // very low life + 3 uncapped resists
